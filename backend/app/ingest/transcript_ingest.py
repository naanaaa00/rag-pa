...
#         "main_topic": summary_metadata.get("main_topic", "General"),
...
def ingest_transcript(text: str, summary_metadata: dict):
    """
    Memproses transkrip dengan Semantic Chunking dan menyimpannya ke PGVector.
    Dilengkapi dengan pembersihan metadata otomatis dari karakter markdown (**).
    """

    # --- Fungsi Internal untuk Membersihkan Metadata ---
    def clean_val(val):
        if isinstance(val, str):
            # Hapus tanda bintang ganda dan spasi di ujung teks
            return val.replace("**", "").strip()
        if isinstance(val, list):
            # Jika metadata berbentuk list (seperti participants), bersihkan tiap isinya
            return [clean_val(v) for v in val]
        return val

    # 1. Ambil dan Bersihkan Metadata (Mencegah Double Summary & Karakter Sampah)
    minimal_metadata = {
        "title": clean_val(summary_metadata.get("title", "Untitled")),
        "date": clean_val(summary_metadata.get("date", "Unknown")),
        "main_topic": clean_val(summary_metadata.get("main_topic", "General")),
        "participants": clean_val(summary_metadata.get("participants", [])),
        "content_type": "transcript",
        "meeting_id": summary_metadata.get("meeting_id") # Pastikan ID ikut jika ada
    }
    ...
    try:
        ...
        final_docs = []
        for i, chunk in enumerate(initial_chunks):
            # Jika chunk terlalu besar, potong paksa agar tidak Error di Ollama
            if len(chunk) > 6000:
                logger.warning(f"Chunk {i} terlalu besar ({len(chunk)} char). Membagi dengan safety net...")
                sub_chunks = safety_splitter.split_text(chunk)
                for j, sub_chunk in enumerate(sub_chunks):
        ...
        # Gunakan batch kecil agar koneksi tidak timeout
        batch_size = 5
        for i in range(0, chunk_count, batch_size):
            batch = final_docs[i:i + batch_size]
            vector_store.add_documents(batch)
